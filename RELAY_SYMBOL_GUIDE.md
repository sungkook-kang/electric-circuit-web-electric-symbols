...
```
    ...
    [설명]
    코일:
    - A1, A2: 코일 단자 (AC220V, DC24V 등 제어전원 연결)
    - K: 릴레이 표시 기호

    접점:
    - a접점(NO): 11-12 단자, 평상시 열림, 코일 ON시 닫힘
    - b접점(NC): 21-22 단자, 평상시 닫힘, 코일 ON시 열림
    - c접점: 공통 단자를 중심으로 NO와 NC가 결합된 전환 접점

    [동작 원리]
    1. 코일(A1-A2)에 정격 전압 인가
    2. 전자석 효과로 접점 구동 아마추어(armature) 동작
    3. a접점: 열림 → 닫힘 / b접점: 닫힘 → 열림
```
